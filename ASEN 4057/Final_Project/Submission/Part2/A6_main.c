...
void A6_intEuler(double *out, double dVx, double dVy, double clearance, int writecommand, int objective, double tolerance){
    ...
    while (endcon == 0){
        ...
        // Moon velocity
        vMx[1] = vMx[0] + ((FEMx[0]+FSMx[0])/mM)*dt;
        vMy[1] = vMy[0] + ((FEMy[0]+FSMy[0])/mM)*dt;
...
#
#########################################################*/
void A6_Grid(double *dVarray, int objective, double clearance, double tolerance,int argc, char *argv[]){
    ...
    if(myrank==1){
        ...
        MPI_Send(&dVmin, 2, MPI_DOUBLE, 2, 0, MPI_COMM_WORLD);
        }
        ...
        MPI_Recv(&dVmin, 2, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        // fine grid search
...
int main(int argc, char *argv[]){
    ...
    A6_Grid(dVarray, objective, clearance, tolerance, argc, argv);
