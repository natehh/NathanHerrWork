...
double RK_ex(int dt, double IC, double func){
    ...
    double ystar1, ydoublestar, ystar, y;
    ystar1 = IC + .5*dt*func;
    ydoublestar = IC + .5*dt*func;
    ystar = IC + dt*func;
    y = IC + (dt/6)*(func + 2*func+2*func+func);
    return y;
}
/*#######################################################
# Nathan Herr and Declan Murray
...
void Final_intEuler(double *out, double dVx, double dVy, double clearance, int writecommand, int objective, double tolerance){
    ...
    endcon = 0;
    i = 1;
    double accelSx, accelSy, accelMx, accelMy;

    // loop to perform Numerical Integration (RK4)
    while (endcon == 0){
        //Spacecraft acceleration
        accelSx = RK_accel(FMSx[0], FESx[0],mS);
        accelSy = RK_accel(FMSy[0], FESy[0],mS);
        // Spacecraft velocity
        /*vSx[1] = vSx[0] + ((FMSx[0]+FESx[0])/mS)*dt;
        vSy[1] = vSy[0] + ((FMSy[0]+FESy[0])/mS)*dt;*/
        vSx[1] = RK_ex(dt, vSx[0], accelSx);
        vSy[1] = RK_ex(dt, vSy[0], accelSy);
        // Moon acceleration
        accelMx = RK_accel(FEMx[0], FSMx[0],mM);
        accelMy = RK_accel(FEMy[0], FSMy[0],mM);
        // Moon velocity
        /*vMx[1] = vMx[0] + ((FEMx[0]+FEMx[0])/mM)*dt;
        vMy[1] = vMy[0] + ((FEMy[0]+FEMy[0])/mM)*dt;*/
        vMx[1] = RK_ex(dt, vMx[0], accelMx);
        vMy[1] = RK_ex(dt, vMy[0], accelMy);

        // Spacecraft Position
        /*xS[1] = xS[0] + vSx[0]*dt;
        yS[1] = yS[0] + vSy[0]*dt;*/
        xS[1] = RK_ex(dt, xS[0], vSx[0]);
        yS[1] = RK_ex(dt, yS[0], vSy[0]);
        // Moon position
        /*xM[1] = xM[0] + vMx[0]*dt;
        yM[1] = yM[0] + vMy[0]*dt;*/
        xM[1] = RK_ex(dt, xM[0], vMx[0]);
        yM[1] = RK_ex(dt, yM[0], vMy[0]);
        ...
        endcon = Final_events(dEM[1], dSM[1], dES[1], clearance, rE, rM, t);
...
#
#########################################################*/
void Final_Grid(double *dVarray, int objective, double clearance, double tolerance, int argc, char *argv[]){
    ...
    if (objective==1){
        ...
        // find bounds
        int locsize = n/nprocs;
        int begin = myrank*locsize;
        int end = begin+locsize;
        if (myrank==1){
            end = n-1;
        }
        for (int i=min; i<=max; i+=gsize){
            for (int j=min; j<=max; j+=gsize){
                // Call Euler's
                Final_intEuler(out, i, j, clearance, writecommand, objective, tolerance);
                endcon = (int)*out;
                //printf("\ni=%d\t\tj=%d\n",i,j);
                // For successful iteration
                if (endcon==2){
                    //printf("\nSuccess on Proc %d Gridsearch\n", myrank);
        ...
        int outsize = 2*nprocs;
        double dVmins[outsize];
        //printf("\nProcess %d: %f\t%f",myrank,dVmin[0],dVmin[1]);
        MPI_Gather(dVmin, 2, MPI_DOUBLE, dVmins, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        /*if (myrank==serverrank){
            printf("\nProc 1: %f\t %f\n", dVmins[0], dVmins[1]);
            printf("\nProc 2: %f\t %f\n", dVmins[2], dVmins[3]);
            printf("\nProc 3: %f\t %f\n", dVmins[4], dVmins[5]);
...
int main(int argc, char *argv[]){
    ...
    Final_Grid(dVarray, objective, clearance, tolerance, argc, argv);
    ...
    // integration call
    int writecommand = 1;
    Final_intEuler(place, dVx, dVy, clearance, writecommand, objective, tolerance);
